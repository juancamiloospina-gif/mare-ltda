...
import { products, categories, type CategorySlug } from "@/data/mare-data";

const speciesOptions: { label: string; slug: CategorySlug }[] = [
  { label: "Bovinos", slug: "bovinos" },
  { label: "Equinos", slug: "equinos" },
];

// Categoría / Laboratorio siguen siendo decorativas — no hay datos reales
// para clasificar por ahí todavía.
const decorativeFilterGroups = [
  { group: "Categoría", options: ["Vacunas", "Antiparasitarios"] },
  { group: "Laboratorio / MSD", options: ["MSD Salud Animal", "Otros laboratorios"] },
];

const routeApi = getRouteApi("/catalogo");

export function CatalogoPage() {
  useReveal();
  const { linea } = routeApi.useSearch();
  const [activeProduct, setActiveProduct] = useState<number | null>(null);
  const [speciesFilter, setSpeciesFilter] = useState<Set<CategorySlug>>(
    () => new Set(linea === "bovinos" || linea === "equinos" ? [linea] : []),
  );

  const comingSoonCategory = categories.find((c) => c.slug === linea && !c.available);

  const toggleSpecies = (slug: CategorySlug) => {
  ...
  const filteredProducts = useMemo(() => {
    if (speciesFilter.size === 0) return products;
    return products.filter((product) => speciesFilter.has(product.category));
  }, [speciesFilter]);
        ...
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
          <div className="grid gap-8 lg:grid-cols-[17rem_1fr]">
            ...
            >
              ...
              {/* TODO: conectar backend — búsqueda por texto todavía no filtra resultados. */}
              ...
              <fieldset className="mt-6 border-t border-border pt-5">
                <legend className="mb-3 font-semibold text-primary">Especie</legend>
                {speciesOptions.map(({ label, slug }) => (
                  <label
                    key={slug}
                  ...
                  >
                    <input
                      ...
                      checked={speciesFilter.has(slug)}
                      onChange={() => toggleSpecies(slug)}
                    />
                    {label}
                  </label>
                ))}
              </fieldset>
              {decorativeFilterGroups.map(({ group, options }) => (
                <fieldset key={group} className="mt-6 border-t border-border pt-5">
                  <legend className="mb-3 flex items-center gap-2 font-semibold text-primary">
                    {group}
              ...
              {filteredProducts.length === 0 ? (
                <div className="flex flex-col items-center gap-3 border border-dashed border-border bg-background px-6 py-20 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-primary">
                    {comingSoonCategory
                      ? `${comingSoonCategory.label} próximamente`
                      : "Sin resultados para este filtro"}
                  </h3>
                  <p className="max-w-sm text-sm text-muted-foreground">
                    {comingSoonCategory
                      ? comingSoonCategory.tagline
                      : "Ajusta la especie seleccionada para ver el resto del portafolio."}
                  </p>
                </div>
              ) : (
